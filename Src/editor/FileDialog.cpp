...
FileDialog::FileDialog(const char* name, const char* ext, eFileDialogUsage usage)
{
	open = false;
	mExt = ext;
...
bool FileDialog::IsExt(const std::string& filename, const std::string& ext)
{
	...
	int lastExt = fn.rfind(e);
	int lastAct = fn.length() - e.length();
		...
		if (mUsage == eFileDialogUsage_OpenFile)
		{
			if (ImGui::Button(STU("打开").c_str(), ImVec2(80, 40)))
			{
				if (fileName.size() == 0)
				{
					...
					if (!IsExt(fileName, mExt))
					{
						fileName = fileName.append(".").append(mExt);
					}
